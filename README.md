...
The action needs an `action-lock.json` file, that holds static information about
how the integrity check will be done.

A built-in example of this file is bundled with this action, it can be
overridden using the input `lock-file-location`.

The action will loop over the `locked-actions`, then matches the mentioned
reference commit against the actual existing one in the repository of the
action.

A report object is produced at the end of the loop, it will be exported as both
of `markdown` and `json` formats
