...
const lockedActionInformation = require('./actions-lock.json') as LockedActionInformation [];

async function getCurrentCommitForActionTagOrBranch(action: string, tagOrBranch: string): Promise<string> {
    let owner = "";
    let repo = "";
    if (action.includes("/")) {
